...
import { NAVIGATION, SIGN_IN_URL } from "@/lib/workspace";
...
export function WorkspaceShell({ children }: { children: React.ReactNode }) {
  const { phase, profile, session, message, can, signOut } = useWorkspace();
  const [navOpen, setNavOpen] = useState(false);
  const pathname = usePathname();

  if (phase === "checking") return <Splash label="Opening your workspace…" />;

  if (phase === "no-profile") {
    return (
      <Splash
        icon="users"
        title="Your employee profile is not set up yet"
        label={message}
 ...
 * Enterprise search.
 *
 * A real form that navigates, not a decorative box — it posts to the knowledge
 * centre, which is the one thing here that is genuinely searchable today.
 * Wiring it to a search that does not exist would be worse than not having it.
 */
function EnterpriseSearch() {
  return (
    <form action="/knowledge" method="get" className="max-w-md flex-1">
      ...
      <div className="relative">
        ...
        <input
          ...
          name="q"
          type="search"
          placeholder="Search policies, SOPs and guidelines"
