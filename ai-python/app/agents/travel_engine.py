...
_PLAN_BY_ID: Dict[str, Dict[str, Any]] = {
    plan["plan_id"]: plan for plan in TRAVEL_PLANS.values()
}
...
def _reason_codes(
    ...
    codes: List[str] = []
    dest      = str(profile.get("destination") or "").strip()
    purpose   = str(profile.get("purpose") or "").strip()
    ages      = str(profile.get("traveller_ages") or "").strip()
    medical   = str(profile.get("medical_conditions") or "").strip()
    visa      = str(profile.get("visa_requirement") or "").strip()
    trip_cost = str(profile.get("trip_cost") or "").strip()

    if dest:
        codes.append(f"Cover matched to their destination: \"{dest}\".")
    if visa:
        codes.append(
            f"Their stated visa requirement — \"{visa}\" — was a hard constraint "
            f"on which plans could qualify."
        )
    if purpose:
        codes.append(f"Rated for the trip they described: {purpose}.")
    if ages:
        codes.append(f"Traveller ages taken into account: {ages}.")
    if medical:
        codes.append(f"Declared medical conditions factored in: \"{medical}\".")
    if trip_cost:
        codes.append(f"Trip cancellation cover sized against the trip cost they gave: {trip_cost}.")
    if not codes:
